...
- 我们提出了第一个基于Transformer的低光图像增强算法——Retinexformer。
    
- 我们提出了一个单阶段的Retinex框架（ORF），其训练过程简单且能够有效建模污染。
...
### 3.1 单阶段 Retinex 框架（One-stage Retinex-based Framework）
...
其中 $⊙$ 表示逐元素乘法。
 ...
 其次，亮化过程（light-up process）可能会放大噪声和伪影，同时还会造成欠曝光/过曝光以及颜色失真，如图 2(a) 中放大的局部区域 i 和 ii 所示。
...
$$
其中 $\hat{R} ∈ R^{H×W×3}$ 和 $\hat{L} ∈ R^{H×W}$ 表示扰动项。
 与 [15,18,49] 类似，我们将 $R$ 视为一个良好曝光的图像。
 为了将 $I$ 亮化，我们对公式 (2) 的两边逐元素相乘一个亮化映射 $\bar{L}$，使得 $\bar{L} ⊙ L = 1$，得到：
$$
I ⊙ \bar{L} = R + R ⊙ (\hat{L} ⊙ \bar{L}) + (\hat{R} ⊙ (L + \hat{L})) ⊙ \bar{L}, \tag{3}
$$
其中 $\hat{R} ⊙ (L + \hat{L})$ 表示隐藏在暗场景中的噪声和伪影，并且会被 $\bar{L}$ 放大；
 而 $R ⊙ (\hat{L} ⊙ \bar{L})$ 表示由亮化过程引起的欠曝光/过曝光及颜色失真。
 我们将公式 (3) 简化为：
$$
...
其中 $I_{lu} ∈ R^{H×W×3}$ 表示亮化后的图像，$C ∈ R^{H×W×3}$ 表示总体损坏项。
 随后，我们将 ORF 表述为：
$$
(I_{lu}, F_{lu}) = E(I, L_p), \quad I_{en} = R(I_{lu}, F_{lu}), \tag{5}
$$
其中 $E$ 表示照明估计器（illumination estimator），$R$ 表示失真恢复器（corruption restorer）。
 $E$ 以 $I$ 及其照明先验图 $L_p ∈ R^{H×W}$ 作为输入，
 其中 $L_p = mean_c(I)$，表示沿通道方向计算每个像素的平均值。
 $E$ 输出亮化图像 $I_{lu}$ 和亮化特征 $F_{lu} ∈ R^{H×W×C}$。
 然后，将 $I_{lu}$ 和 $F_{lu}$ 输入 $R$，以恢复损坏并生成增强图像 $I_{en} ∈ R^{H×W×3}$。

------

**照明估计器（E）的结构**如图 2(a)(i) 所示。
 E 首先使用一个 $1×1$ 卷积将 $I$ 和 $L_p$ 的拼接结果进行融合。
 我们注意到，良好曝光的区域可以为欠曝光区域提供语义上下文信息。
 因此，采用一个**深度可分离卷积（depth-wise separable conv5×5）**来建模不同光照区域间的交互，以生成亮化特征 $F_{lu}$。
 接着，E 使用一个 $1×1$ 卷积来聚合 $F_{lu}$，以生成亮化映射 $\bar{L} ∈ R^{H×W×3}$。
 我们将 $\bar{L}$ 设为三通道 RGB 张量，而不是像 [15,18] 那样使用单通道映射，以提高其跨 RGB 通道建模非线性的能力，从而实现更好的颜色增强。
 然后，使用 $\bar{L}$ 在公式 (3) 中对 $I$ 进行亮化。

------

**讨论（Discussion）**
 (i) 与以往的 Retinex 深度学习方法 [30,49,54,65,66] 不同，我们的 ORF 估计的是 $\bar{L}$ 而非照明图 $L$，
 因为如果 ORF 估计 $L$，则亮化图像需通过逐元素除法 $I ./ L$ 获得。
 计算机在处理极小值（甚至为0）的张量时容易出错，除法操作可能导致数据溢出。
 此外，计算机随机产生的微小误差也会被该除法操作放大，从而导致估计不准确。
 因此，建模 $\bar{L}$ 更为稳健。
 (ii) 先前的 Retinex 深度学习方法主要关注抑制反射图像上的噪声（即公式 (2) 中的 $\hat{R}$），
 但忽略了照明图估计误差（即公式 (2) 中的 $\hat{L}$），从而在亮化过程中容易产生欠曝光/过曝光和颜色失真。
 相比之下，我们的 ORF 同时考虑了这些损坏，并使用 $R$ 来恢复它们。
...
**网络结构（Network Structure）**
 如图 2(a)(ii) 所示，IGT 采用三尺度的 U 形结构 [44]。
 IGT 的输入为亮化图像 $I_{lu}$。
