...
所以怎么解决？需要设置相应的路由策略来解决问题。这里我选择让各**边缘交换机**对上层汇聚交换机启用`allowas-in 1`，这样edge就可以接受那些起源于同pod的edge/AS-Path中已经包括本ASN一次的路由。至于**汇聚/核心交换机**，因为它们并不会收到来自同一层同一pod的交换机发来的包，所以也就不会出现拒收的问题，因此不用设置。
